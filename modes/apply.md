...
Interactive mode for when the candidate is filling out an application form in Chrome. Reads what's on screen, loads previous offer context, and generates personalized responses for each form field.
...
- **Update tracker**: `node db.mjs update application {id} --field role --value "{new_role}" --json`
...
1. Update status in DB: `node db.mjs update application {id} --field status --value Applied --json`

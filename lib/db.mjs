...
export function insertApplication(db, data) {
  ...
  try {
    return db.transaction((tx) => {
      const row = tx.select({ max: max(applications.num) }).from(applications).get();
      ...
      return { id: Number(result.lastInsertRowid), num };
    });
  } catch (err) {
...
export function insertPipelineEntry(db, data) {
  try {
    const result = db.insert(pipelineEntries).values({
      url: data.url,

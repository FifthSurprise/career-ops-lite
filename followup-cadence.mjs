 ...
 *
 * Reads from SQLite DB (applications) + data/follow-ups.md (markdown, stays as-is),
 * calculates follow-up cadence for active applications, and flags overdue entries.
 *
...
function today() { return new Date(new Date().toISOString().split('T')[0]); }
function parseDate(s) { if (!s || !/^\d{4}-\d{2}-\d{2}$/.test(s.trim())) return null; return new Date(s.trim()); }
function daysBetween(d1, d2) { return Math.floor((d2 - d1) / (1000 * 60 * 60 * 24)); }
function addDays(date, days) { const r = new Date(date); r.setUTCDate(r.getUTCDate() + days); return r.toISOString().split('T')[0]; }
...
function parseFollowups() {
  ...
  for (const line of readFileSync(FOLLOWUPS_FILE, 'utf-8').split('\n')) {
    ...
    entries.push({ num, appNum: parseInt(parts[2]), date: parts[3], company: parts[4], role: parts[5], channel: parts[6], contact: parts[7], notes: parts[8] || '' });
  }
  return entries;
}
...
function extractContacts(notes) {
  ...
  for (const email of emails) {
    ...
    const nameMatch = before.match(/(?:Emailed|emailed|contact[:\s]+|to\s+)([A-Z][a-z]+ ?[A-Z]?[a-z]*)\s*(?:at|@|$)/i);
...
function computeNextFollowupDate(status, appDate, lastFollowupDate, followupCount) {
  ...
  if (status === 'interview') return addDays(parseDate(appDate), CADENCE.interview_thankyou);
  return null;
}
...
function analyze() {
  const db = openDb();
  initSchema(db);
  const apps = listApplications(db, { statusIn: ['Applied', 'Responded', 'Interview'] });

  if (apps.length === 0) return { error: 'No actionable applications found in DB.' };
  ...
  for (const app of apps) {
    ...
    entries.push({
      num: app.num, date: app.date, company: app.company, role: app.role,
      status: normalized, score: app.score, notes: app.notes,
      reportPath: app.report_path,
      contacts: extractContacts(app.notes),
      daysSinceApplication: daysSinceApp, daysSinceLastFollowup,
      followupCount, urgency, nextFollowupDate, daysUntilNext,
    });
  }
  ...
  const filtered = overdueOnly ? entries.filter(e => e.urgency === 'overdue' || e.urgency === 'urgent') : entries;

  const totalTracked = listApplications(db, {}).length;
...
function printSummary(result) {
  ...
  console.log(`\n${'='.repeat(70)}\n  Follow-up Cadence Dashboard — ${metadata.analysisDate}\n  ${metadata.totalTracked} total applications, ${metadata.actionable} actionable\n${'='.repeat(70)}\n`);
  if (entries.length === 0) { console.log('  No active applications to track.\n'); return; }
  ...
  for (const e of entries) {
    ...
    console.log('  ' + String(e.num).padEnd(5) + e.company.substring(0, 15).padEnd(16) + e.status.padEnd(12) + String(e.daysSinceApplication).padEnd(6) + String(e.followupCount).padEnd(5) + (e.nextFollowupDate || '-').padEnd(13) + urgLabel.padEnd(10) + contactStr);
  }
  console.log('');
}
